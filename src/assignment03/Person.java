...
//***********************************************************
	...
	/**
	 * returns the Person with the longest lastNames field in an array of Person elements
	 * @param array an array of person elements
	 */
	public static Person longestLastName(Person[] array) {
		Address add = new Address("","","","","");
		Person per = new Person("","",add,"012-345-6789","2000-01-01");
		int longestln = 0;
		for(int i = 0; i < array.length; i++) {
			if(array[i].getLastNames().length() > longestln) {
				per = array[i];
				longestln = array[i].getLastNames().length();
			}
		}
		return per;
	}
